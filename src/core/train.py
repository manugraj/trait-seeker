...
from keras.layers import Dense, LSTM, SpatialDropout1D, Conv1D, MaxPooling1D, Input
...
class Train:
    ...
    class NLTKPreprocessor(BaseEstimator, TransformerMixin):
        ...
        def vectorize(self, doc):
            ...
            """
            save_path = paths(Constants.DATA_PATH, "padding.pickle")
    ...
    def load_google_vec(self):
        ...
        return KeyedVectors.load_word2vec_format(
            paths(Constants.DATA_PATH, 'GoogleNews-vectors.bin.gz'),
            binary=True)
    ...
    def lemmatize_token(self, token, tag):
        ...
        return WordNetLemmatizer().lemmatize(token, tag_processed)
    ...
    def run(self, X, y, model_name=None, pretrained_weights_path=None, pretrained_model_path=None, verbose=True):
        ...
        # Save the model
        if model_name:
            outpath = Constants.MODEL_PATH
            classifier.save_weights(os.path.join(outpath, model_name + '.h5'))
            with open(os.path.join(outpath, model_name + '.json'), 'w') as json_file:
